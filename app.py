...
def get_facenet_embedding(img_array, facenet_resnet, tf):
    ...
    embedding = facenet_resnet(input_tensor_batch, False, False, momentum_tensor)
    return embedding.numpy()[0]

def get_vit_features(img_array, vit_extractor, vit_model, torch):
...
async def predict(file: UploadFile = File(...)):
    ...
    try:
        ...
        facenet_feat = get_facenet_embedding(img_array, facenet_resnet, tf)
        vit_feat = get_vit_features(img_array, vit_extractor, vit_model, torch)
